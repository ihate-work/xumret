...
- Always import from `~/api`. Never import from `webui-src/api/generated/` directly — that folder is regenerated by `make openapi` and treated as build output.
- After changing a FastAPI route or Pydantic model, run `make openapi` to refresh both `webui-src/api/openapi.yaml` and `webui-src/api/generated/**` and commit both.
